<script lang="ts">
	...
	$: submitSize = readyToSubmit ? 'default' : 'sm';

	onMount(() => {
		const { email: oldEmail, password: oldPassword } = JSON.parse(
...
<form use:enhance class="mx-auto mt-8 flex max-w-[330px] flex-col px-8 py-4 shadow" method="post">
